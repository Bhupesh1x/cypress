...
const CreateElection = ({ user }: { user: IElectionAdmin }) => {
  ...
  const electionSchema = Yup.object().shape({
    ...
    pollsTimezone: Yup.string().required('Required').test('valid-timezone-check', 'Must be a valid timezone', (value) => Object.values(timezones).includes(value.toUpperCase())),
